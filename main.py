...
def print_intro():
    ...
    try:
        ...
        print(f.renderText("Grass 🍀 V3.11"))
    ...
    print(Fore.CYAN + "👨‍💻 Created by: OstinZ" + Style.RESET_ALL)  # Cyan color for the creator
...
def main():
    ...
    try:
        with open('uid.txt', 'r') as uid_file:
            user_ids = [line.strip() for line in uid_file if line.strip()]
    except FileNotFoundError:
        logger.error("uid.txt file not found.")
        return
    ...
    num_accounts = len(user_ids)  # Use the number of IDs from the uid.txt file

    # Check if we have enough proxies
    required_proxies = num_accounts * 3  # Each user_id now requires only 3 proxies
    ...
    for user_id in user_ids:
        # Assign 3 proxies to this user (changeable to adjust proxy usage per user)
        user_proxies = proxy_list[proxy_index:proxy_index + 3]
        proxy_index += 3
